...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filesname, 'rb') as imgpath:
        ...
        X = np.frombuffer(imgpath.read(), dtype=np.uint8).reshape(len(y), 784)
...
def softmax_loss(Z, y_one_hot):
    ...
    # Compute log(sum(exp(Z[i, :]))) for each example in the batch
    log_sum_exp = ndl.log(ndl.summation(ndl.exp(Z), axes=(1,)))
    
    # Z * y_one_hot gives us Z[i,j] where j is the true class, 0 elsewhere
    true_class_logits = ndl.summation(Z * y_one_hot, axes=(1,))
    
    loss_per_example = log_sum_exp - true_class_logits
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        ...
        hidden = ndl.relu(X_tensor @ W1)  # Apply ReLU after first layer
        logits = hidden @ W2              # Second layer (no activation)
