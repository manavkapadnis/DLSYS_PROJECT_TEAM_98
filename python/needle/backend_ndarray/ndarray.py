...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
      ...
      if neg_idx != -1:
          ...
          for i, dim in enumerate(new_shape):
              if i != neg_idx:
                  known_size *= dim
          new_shape[neg_idx] = prod(self.shape) // known_size
      ...
      # Create a new NDArray with the same backing but different shape
      new_array = NDArray.make(new_shape, strides=None, device=self.device, handle=self._handle, offset=self._offset)
      return new_array
      ### END YOUR SOLUTION
    ...
    def permute(self, new_axes: tuple[int, ...]) -> "NDArray":
        ...
        if len(new_axes) != self.ndim:
          raise ValueError("new_axes must have the same length as number of dimensions")
    
        if set(new_axes) != set(range(self.ndim)):
          raise ValueError("new_axes must be a permutation of dimension indices")
        
        # Permute the shape and strides according to new_axes
        new_shape = tuple(self.shape[i] for i in new_axes)
        new_strides = tuple(self.strides[i] for i in new_axes)
        
        # Return a new NDArray with permuted shape and strides
        return NDArray.make(
            shape=new_shape,
            strides=new_strides,
            device=self.device,
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if len(new_shape) < self.ndim:
            raise ValueError("new_shape must have at least as many dimensions as original shape")
        
        # Pad the current shape with 1s on the left to match new_shape length
        old_shape = (1,) * (len(new_shape) - self.ndim) + self.shape
        old_strides = (0,) * (len(new_shape) - self.ndim) + self.strides
        
        # Check compatibility and compute new strides
        new_strides = []
        for i in range(len(new_shape)):
            if old_shape[i] == new_shape[i]:
                # No broadcasting needed, keep the stride
                new_strides.append(old_strides[i])
            elif old_shape[i] == 1:
                # Broadcasting dimension, stride becomes 0
                new_strides.append(0)
            else:
                raise ValueError(f"Cannot broadcast dimension {i} from {old_shape[i]} to {new_shape[i]}")
        
        # Return a new NDArray with broadcast shape and adjusted strides
        return NDArray.make(
            shape=new_shape,
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, slc in enumerate(slices):
            ...
            # Compute new stride (multiply by step)
            new_strides.append(self.strides[i] * step)
        
        # Return a new NDArray with adjusted shape, strides, and offset
        return NDArray.make(
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        # Calculate slices for where to place original data
        slices = tuple(
