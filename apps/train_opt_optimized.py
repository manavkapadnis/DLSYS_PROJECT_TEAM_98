"""
Memory-Optimized Training Script for OPT-125M
...
def train(model, train_data, val_data, config, n_epochs=10, batch_size=8, 
          seq_len=256, lr=6e-4, device=None, checkpoint_dir=None, 
          accumulation_steps=4):
    """Main training loop with gradient accumulation"""
    print("=" * 80)
    print("OPT-125M TRAINING CONFIGURATION")
    print("=" * 80)
    print(f"Model: OPT-125M (~{config.get_total_params() / 1e6:.1f}M parameters)")
    print(f"Sparse attention: {config.use_sparse_attention}")
    if config.use_sparse_attention:
        print(f"Pattern: {config.sparse_pattern}")
    ...
    # Optimizer (using OPT paper's recommended settings)
    optimizer = ndl.optim.Adam(model.parameters(), lr=lr, weight_decay=0.1)
    ...
    for epoch in range(n_epochs):
        ...
        if checkpoint_dir and val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(checkpoint_dir, 'best_opt_model.pkl')
...
def main():
    parser = argparse.ArgumentParser(description='Train OPT-125M')
